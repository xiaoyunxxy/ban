...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=8, norm_layer=None):
    ...
    #     flip_mask = torch.load('/home/xxu/b_detection/bd_exp/mitigation_pth/featureRE_bpp_all2all_cifar10_mask.th')

    #     strategy = "flip"
    #     features = self.from_input_to_features(x, 0)
    #     if strategy == "flip":
    #         features = (1 - flip_mask) * features - flip_mask * features
    #     elif strategy == "zero":
    #         features = (1 - flip_mask) * features

    #     out = self.from_features_to_output(features, 0)
    ...
    def from_input_to_features_tsne(self, x, index):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
    ...
    def forward_allfeatures(self, x):
        fea_out = []
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        fea_out.append(out)
